...
int empty1[Max_Proc];//可推出空的非终结符的编号  
...
{  
    ...
    for(int i=0; i<nonTerMap.size(); i++)  
    {  
        cout << "First[%s] = " << nonTerMap[i].first;  
        for(int j=0;; j++)  
        {  
            if(first[i][j] == -1)  
            {  
...
{  
    ...
    for(int i=0; i<nonTerMap.size(); i++)  
    {  
        cout << "Follow[%s] = " << nonTerMap[i].first;  
        for(int j=0;; j++)  
        {  
            if(follow[i][j] == -1)  
            {  
...
{  
    ...
    resultfile.open("preciateResult.txt", stdout);  
