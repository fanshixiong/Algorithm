...
void MAlex::print_list(string &s1)   //输出表格
{
	char buffer[128];
	s1 = "界符编号1-12\r\n";
...
//尝试识别不合法的定义，即int int/int char/char int 等
/*void MAlex::cope_firsrtaph(int &i) {   //处理首字符是字母
	int num = 0;
	int j = i - 1;
	recog_reserve(i, num);
	if (num) {
		if (num-32 == 7 || num-32 == 8 || num-32 == 9 || num-32 == 10) {//当保留字为int/char/string/bool时，进行是否存在错误定义的判断
			while (line[j] == ' ') j--;//回溯，除空格
			if ((line[j - 2] == 'i' && line[j - 1] == 'n' && line[j] == 't') ||
				(line[j - 3] == 'c' && line[j - 2] == 'h' && line[j - 1] == 'a' && line[j] == 'r') ||
				(line[j - 5] == 's' && line[j - 4] == 't' && line[j - 3] == 'r' && line[j - 2] == 'i' && line[j - 1] == 'n' && line[j] == 'g') ||
				(line[j - 3] == 'b' && line[j - 2] == 'o' && line[j - 1] == 'o' && line[j] == 'l'))
				resout << '<' << num << ',' << key[num - 32] << ',' << "保留字" << ',' << "错误的定义" << '>' << " ";
		}
		else
			resout << '<' << num << ',' << key[num - 32] << ',' << "保留字" << '>' << "  ";
		preout << key[num - 32];
...
/*	int j = 0;
	...
	int j = 0;
	resout << '<';
	if (is_alpbat(line[i + 1])) {//以数字开头的标识符，即不合法的标识符
		for (; is_digit(line[i]) || is_alpbat(line[i]); i++) {
			preout << line[i];
			resout << line[i];
		}
		resout << ',' << "不合法的标识符" << '>' << " ";
